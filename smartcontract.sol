...
contract Vote {
    ...
    //Election commission of the voting contract(msg.sender)
    ...
    modifier isValidAge(uint _age){
        require(_age>=18,"You are not eligible to caste vote");
        _;
    }
   ...
   return voterList;
    ...
    function casteVote(uint _voterId, uint _candidateId) external isVotingOver(){
    require (gldToken.balanceOf(msg.sender)>0,"Not allowed");
    require(block.timestamp>=startTime,"Voting yet not started");
